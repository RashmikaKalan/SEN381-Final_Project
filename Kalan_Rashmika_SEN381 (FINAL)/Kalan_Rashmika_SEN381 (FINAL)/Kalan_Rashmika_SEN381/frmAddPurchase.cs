...
namespace Kalan_Rashmika_SEN381
{
    public partial class frmAddPurchase : Form
    {
        ...
        Contracts cons = new Contracts();
        MonthlyBilling bills = new MonthlyBilling();
        ClientAddOns cadd = new ClientAddOns();
                    ...
                    if (clients.Any(client => client.IDNum == txtID.Text))
                    {
                        foreach (var person in clients)
                        {
                            if (person.IDNum == txtID.Text)
                            {
                                ...
                                Subscriptions subscription = new Subscriptions(2, person.ID, prodID, version, total, DateTime.Now, Subscriptions.GenerateSerialNumber());
                                ...
                                foreach (var item in newsubs)
                                {
                                    if (item.ClientID==person.ID && item.ProdID==prodID)
                                    {
                                        ...
                                        if (cbSensor.Checked == true)
                                        {
                                            ClientAddOns newa = new ClientAddOns(addOns[0].ID, item.ID, int.Parse(nudSensor.Value.ToString()));
                                            cao.InsertAddOns(newa);
                                        }
                                        if (cbActor.Checked == true)
                                        {
                                            ClientAddOns newa2 = new ClientAddOns(addOns[1].ID, item.ID, int.Parse(nudActor.Value.ToString()));
                                            cao.InsertAddOns(newa2);
                                        }
                                        if (cbController.Checked == true)
                                        {
                                            ClientAddOns newa3 = new ClientAddOns(addOns[2].ID, item.ID, int.Parse(nudController.Value.ToString()));
                                            cao.InsertAddOns(newa3);
                                        }
                                    }
                                }
